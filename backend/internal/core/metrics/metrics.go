...
// image to label next.
//
// Ported from backend/tools/metrics.py. That file stays in the repo and
// stays runnable, because tools/experiment_conf.py (the T-01 threshold sweep) calls
...
// backend/tests/testdata/metrics_cases.json is what keeps them from drifting apart.
package metrics
