...
- **หนึ่งคนถือได้หนึ่งภาพต่อหนึ่งโปรเจกต์** — จองภาพใหม่ = ปล่อยภาพเดิมอัตโนมัติ จึงไม่ต้องมี endpoint `release` แยกสำหรับการเดินคิว · `release: true` มีไว้สำหรับ "ทำเสร็จแล้ว" ซึ่ง `POST /api/label` เรียกให้เองหลัง save สำเร็จ
