...
**Prompt bank** ที่เก็บ embedding สะสมของทุกคลาสที่ผู้ใช้เคย label ไว้ด้วยมือ เก็บที่ `<output_dir>/_bank/` ยิ่งสะสมมาก โมเดลยิ่งแยกแยะคลาสนั้นได้แม่นขึ้น (implement เป็น class `Bank` ใน `services/bank.py`)
...
**Test set / Ground truth** ชุดภาพที่แยกเก็บต่างหาก (`test_dir`) พร้อมป้ายที่มนุษย์วาดไว้เป็นคำตอบจริง **ไม่เคยถูกป้อนเข้า prompt bank** ใช้วัดผลเท่านั้น
