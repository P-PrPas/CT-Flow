...
## คำที่ Phase 2 เพิ่มเข้ามา (ดู [PHASE2_WORKSPACE.md](./PHASE2_WORKSPACE.md))
...
**Claim (การจองภาพ)** การบอกคนอื่นว่า "ฉันกำลังทำภาพนี้อยู่" · เก็บใน memory (`internal/platform/claims`) TTL 10 นาที หนึ่งคนถือได้หนึ่งภาพต่อหนึ่งโปรเจกต์ · **เป็นคำแนะนำ ไม่ใช่ล็อก** — มันทำให้คิวของแต่ละคนไม่ชี้ไปที่ภาพเดียวกัน แต่ `POST /api/label` ไม่เคยปฏิเสธเพราะเรื่องจอง (การปฏิเสธ save จะทิ้งกล่องที่วาดไปแล้ว) · เดินไปภาพถัดไป = ปล่อยภาพเดิม, save = ปล่อย, จองซ้ำของตัวเอง = ต่ออายุ

**`oid` vs `username`** สองอย่างนี้ทำคนละหน้าที่และห้ามสลับกัน — `oid` คือ `sub` จาก OIDC ใช้ **เทียบ** ("โปรเจกต์นี้ของฉันไหม", "ใครถือภาพนี้") เพราะมันไม่เปลี่ยนตอนคนถูกเปลี่ยนชื่อ · `username` ใช้ **แสดง** เท่านั้น · `store.UserNames` เป็นที่เดียวที่แปลง `oid` → ชื่อ · **กับดัก:** ภายใต้ local login สองอย่างนี้เป็น string เดียวกัน โค้ดที่สับสนจึงผ่านทั้ง dev และ CI แล้วไปพังบน OIDC จริงเท่านั้น
