...
14. **ย้าย annotation storage (label/box) จากไฟล์ YOLO txt ไปเป็นตาราง PostgreSQL + เพิ่ม export format selection (YOLO/COCO/VOC)** — งานแทรกที่ตกลงกับทีมแล้ว (2026-08-21) แรงจูงใจคือรองรับหลายคนแก้ project เดียวกันพร้อมกัน (เตรียมทางสำหรับ login + workspace แบบ Label Studio ในอนาคต) และวางรากฐาน DB ตามที่ทีม infra ต้องการ — **scope เฉพาะ label/box metadata เท่านั้น `embeddings.pt` ยังเป็นไฟล์เหมือนเดิม** ยังไม่ implement รอคำสั่งเริ่มงาน ดูแผนเต็มที่ [DB_MIGRATION_PLAN.md](./DB_MIGRATION_PLAN.md)
