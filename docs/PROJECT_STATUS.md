...
**หลัง port เป็น Go มีสามชั้น:** (1) `go test ./...` — unit test ต่อ package รวม `internal/store` ที่รันกับ PostgreSQL จริง (2) `backend/_smoke_test.py` — end-to-end ยิงผ่าน HTTP จริง (3) `backend/testdata/` — golden vector ข้ามภาษาที่ Go ต้อง reproduce ให้ตรงเป๊ะ

**smoke test ไม่ใช่ pytest** เป็นสคริปต์รันตรง (`SMOKE_BASE_URL=... python -m backend._smoke_test`) ยิงผ่าน endpoint จริงทั้งหมด **ของ backend ตัวไหนก็ได้ที่ฟังอยู่ที่ URL นั้น** — นี่คือสิ่งที่ทำให้ "Go ทำงานเหมือน FastAPI" พิสูจน์ได้ด้วยคำสั่งเดียว ไม่ใช่ suite ที่สองที่เพี้ยนจากกันได้ · ตรวจสถานะ PostgreSQL ผ่าน `backend/_dbcheck.py` (คุยกับ DB ตรง ๆ ไม่ import โค้ดที่ตัวเองกำลังตรวจ) และตรวจ prompt bank ผ่าน `backend/_bank_test.py` แยกต่างหาก ครอบคลุมพฤติกรรมสำคัญต่อไปนี้ (T-21, 2026-08-21: อัปเดตให้ตรงกับ label storage ที่ย้ายไป PostgreSQL แล้ว — ทดสอบผ่านจริงกับ Postgres จริงในเซสชันนี้):
...
ฝั่ง Python ที่เหลือมี self-check ของตัวเอง: `python -m backend.services.{models,metrics,groundtruth}`, `python -m backend._dbcheck`, `python -m backend._gen_testdata --check`
...
- **Job tracker เก็บสถานะในหน่วยความจำ process เดียว.** ไม่ persist ข้าม restart, ไม่มี TTL/eviction ของ job เก่า — **port มาจาก `job_tracker.py` แบบไม่แก้พฤติกรรมโดยตั้งใจ** (`internal/jobs`) มีคอมเมนต์ `ponytail:` ระบุทางแก้เป็น Redis/TTL ไว้แล้ว
