...
[1] เปิด session          → เลือก input_dir เดียวผ่าน DirPicker (output/test folder ถูกยกเลิกแล้ว — ระบบจัดการเองใต้ .ctflow/)
       ...
       ↓                     ✅ แก้แล้วใน T-29: เปิดจาก `/` แล้วอยู่ที่ `/p/{id}` — URL คือตัวจำ ไม่ใช่ localStorage
       ↓                     ✅ แก้แล้วใน T-29: `/` ตอบได้ว่ามีงานอะไร ใครเป็นเจ้าของ ไปถึงไหน
[2] วาดกล่อง + Save       → สร้าง SAVPE embedding เข้า bank
       ↓                     ✅ อีกคนที่ทำโปรเจกต์เดียวกันไม่ถูกเสนอภาพเดียวกันแล้ว (FR-48/FR-49)
...
| ID | Requirement | สถานะ | หมายเหตุ |
|---|---|---|---|
| FR-43 | สร้าง / ดู / เปลี่ยนชื่อ / ลบ โปรเจกต์ได้จาก UI โดยระบุชื่อ, โฟลเดอร์ dataset และชนิดงาน | ✅ | API (T-26) + UI บน home (T-29): dialog สร้าง · rename inline · claim เมื่อยังไม่มีเจ้าของ · ลบ = ลบแถวใน DB เท่านั้น **ไม่แตะไฟล์บนดิสก์** |
| FR-44 | Home page แสดงรายการโปรเจกต์ทั้งหมด พร้อมเจ้าของ ความคืบหน้า และคนที่ลงมือทำ | ✅ | `/` คือหน้ารายการ (T-29) แบ่ง "Yours"/"Everyone else's" ด้วย `owner.oid` เทียบกับ `oid` ของตัวเองจาก `GET /api/auth/me` **ไม่ใช่ชื่อที่แสดง** (ชื่อซ้ำกันได้และเปลี่ยนได้) · ไม่ซ่อนโปรเจกต์ของใครจากใคร · `labeled`/`auto` มาจาก SQL count ไม่มีการอ่านโฟลเดอร์ |
| FR-45 | รองรับชนิดงาน label แบบอื่นในอนาคตโดยไม่ต้องรื้อของเดิม | 🟡 | คอลัมน์ `task_type` + โครง routing ครบแล้ว (T-26/T-30): `app/modules/detection/` แยกออกจากของกลาง และ `workspaceHref()` คือจุดแตกแขนง **บังคับด้วย CI** ว่าของกลางห้าม import จาก `modules/` · ยัง 🟡 เพราะยังไม่มีโมดูลที่สองมาพิสูจน์ว่าเส้นแบ่งวางถูก · **ไม่มี abstraction** โดยตั้งใจ |
| FR-46 | แต่ละโปรเจกต์มี URL ของตัวเองที่แชร์กันได้ | ✅ | route `/p/{id}` (T-29) resolve id → `input_dir` ตอน mount แล้วส่งต่อให้ `useSession` · `localStorage` (`DIRS_KEY`) ถูกลบทิ้ง URL คือตัวจำแทน และแชร์ข้ามเครื่องได้ — `id` เป็นกุญแจสำหรับอ้างถึง ส่วน `input_dir` ยังเป็นกุญแจสำหรับเก็บ |
...
| FR-48 | เห็นความคืบหน้าของคนอื่นในโปรเจกต์เดียวกันโดยไม่ต้อง refresh | ✅ | T-31 · `GET /api/state` poll ทุก 15 วินาที · หยุดเมื่อ tab ไม่ active · ไม่ใช้ websocket · **ไม่มี `BankSummary` ใน response** เพราะ `classes`/`model` เปลี่ยนเฉพาะตอนตัวเอง label ซึ่ง save คืนให้อยู่แล้ว |
| FR-49 | สองคน label โปรเจกต์เดียวกันแล้วไม่ถูกเสนอภาพเดียวกัน | ✅ | T-32 · `internal/platform/claims` TTL 10 นาที · หนึ่งคนหนึ่งภาพต่อโปรเจกต์ (เดินหน้า = ปล่อย) · save = ปล่อย · จองซ้ำของตัวเอง = ต่ออายุ (heartbeat ทุก 5 นาที) · **เป็นคำแนะนำ ไม่ใช่ล็อก** — `POST /api/label` ไม่เคยปฏิเสธเพราะเรื่องจอง และ `nextTodo` ยอมคืนภาพที่ถูกจองถ้าภาพที่เหลือถูกจองหมด |
...
| NFR-10 | **เข้าถึงได้ตามมาตรฐาน WCAG 2.2 AA** | 🟡 | T-37 · audit เต็มและตัวเลข contrast ทุกตัวคำนวณจาก token ใน `globals.css` อยู่ที่ [UX_AUDIT.md](./UX_AUDIT.md) — คะแนนตั้งต้น **52/100** (fail Level A 11 ข้อ, AA 6 ข้อ) · Phase 1–3 ใส่ทางแก้ครบทุกข้อที่ audit เจอ **ยกเว้นหนึ่ง** ดู NFR-11 · ⚠️ **ยัง 🟡 เพราะยังไม่ได้รัน manual check สักตัว** (axe DevTools, ถอดเมาส์, NVDA/VoiceOver, 320px + zoom 200%, bookmarklet ของ 1.4.12) — คำกล่าวอ้างตอนนี้คือ "ทุกข้อที่เจอมีทางแก้แล้ว" ไม่ใช่ "ผ่าน AA" · `tsc`/`build`/boundary check จับอะไรในงานนี้ไม่ได้เลยสักอย่าง |
| NFR-11 | สร้างกล่องใหม่ด้วยคีย์บอร์ดอย่างเดียวได้ (WCAG 2.1.1 Level A) | ❌ | **fail Level A ที่ merge เข้า main ทั้งที่รู้ตัว** · `BoxCanvas` แก้/ย้าย/ลบกล่องที่มีอยู่ด้วยคีย์บอร์ดได้แล้ว (2.5.7 ผ่าน) แต่การ *วาดกล่องแรก* ยังต้องใช้ pointer · keyboard crosshair เคยลงใน Phase 2 แล้วถูกถอดออกตามที่เจ้าของโปรเจกต์ขอ — ไม่มีใครใช้ และ path ที่ไม่มีคนเดินกลาง hot loop มีต้นทุนดูแลสูงกว่าที่ได้คืน · ทางปิดที่ถูกกว่าอยู่ใน [UX_AUDIT.md](./UX_AUDIT.md) F-01 · **เงื่อนไขเริ่มงาน:** มีผู้ใช้คีย์บอร์ดอย่างเดียวจริง หรือมีข้อผูกพันเรื่อง compliance |
...
1. ~~**ไม่มีแนวคิด "โปรเจกต์" ในสายตาผู้ใช้**~~ → **แก้แล้วใน Phase 2 ก้อนที่ 1–2 (FR-43, FR-44, FR-46, FR-47)** · ที่เหลือของ Phase 2 คือการรับรู้ว่ามีคนอื่นทำงานอยู่ (FR-48…FR-51)
